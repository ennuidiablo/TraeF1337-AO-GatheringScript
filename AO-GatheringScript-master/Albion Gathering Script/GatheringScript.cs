...
namespace Ennui.Script.Official
{
    ...
    public class GatheringState : StateScript
    {
        ...
        public override bool OnStart(IScriptEngine se)
        {
            ...
            AddState("ppone", new PathPoint1State(config, context));
            AddState("pptwo", new PathPoint2State(config, context));
            AddState("ppthree", new PathPoint3State(config, context));
            AddState("pponeb", new PathPoint1BState(config, context));
            AddState("pptwob", new PathPoint2BState(config, context));
            AddState("ppthreeb", new PathPoint3BState(config, context));
            AddState("walkin", new WalkInState(config, context));
            AddState("walkout", new WalkOutState(config, context));
            EnterState("config");
        ...
        public override void OnPaint(IScriptEngine se, GraphicContext g)
        {
            if (config.debugInfo)
            {
                ...
                g.DrawString("http://ennui.nina - Albion Gatherer", theEqualizer, 100);
                ...
                g.DrawString(string.Format("Return to 1st Roam Point: {0}", config.roamPointFirst), theEqualizer, 235);
                ...
                if (config.PP1Area != null && config.PP1Name == Game.ClusterName)
                {
                    config.PP1Area.RealArea(Api).Render(Api, Color.Yellow, Color.Yellow.MoreTransparent());
                }

                if (config.PP2Area != null && config.PP2Name == Game.ClusterName)
                {
                    config.PP2Area.RealArea(Api).Render(Api, Color.Yellow, Color.Yellow.MoreTransparent());
                }

                if (config.PP3Area != null && config.PP3Name == Game.ClusterName)
                {
                    config.PP3Area.RealArea(Api).Render(Api, Color.Yellow, Color.Yellow.MoreTransparent());
                }
